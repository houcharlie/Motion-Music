...
public class AudioPlayer {
	...
	public AudioPlayer(String name, double bpm) {
		this.name = name;
		// allows us to keep track of notes up to sixteenth notes, which are as
		// far as songs go usually
		this.bpm = bpm * 4;
	}
	...
	public void play() {
		...
		for (int i = 0; i < songArray.length; i += samplingWindow) {
			...
			for (int j = i; j < i + samplingWindow * Math.pow(1.5, -tempo)
					&& j < songArray.length; j++) {

				tempo = listener.getTempo();
				volume = listener.getVolume();
				if (j > i + samplingWindow) {
					StdAudio.play(Math.pow(1.05, volume)
